 ...
 */
export async function startRun(agent, scheduleId, input, triggeredAt = new Date()) {
  if (!dbConfigured()) return null;
  const { rows } = await query(
    `insert into schedule_run (agent, schedule_id, triggered_at, status, input)
          values ($1, $2, $3, 'running', $4)
       returning id`,
    [agent, scheduleId, triggeredAt.toISOString(), input],
  );
...
export async function listRuns(agent, limit = 50) {
  ...
  const { rows } = await query(
    `select id, agent, schedule_id, status, error,
...
export async function readRun(agent, id) {
  ...
  const { rows } = await query(
    `select id, agent, schedule_id, status, input, result, error,
            extract(epoch from triggered_at) * 1000 as triggered_at,
            extract(epoch from finished_at) * 1000 as finished_at
       from schedule_run
